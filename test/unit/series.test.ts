...
import {
    createSeries,
    transformSeries,
    validMatrixRowLength,
    reduceMatrixToSeries,
    transposeMatrix,
    mergeRows,
    findIntervalIndexByDecimal,
    getAllIntervals,
    splitPointSeries,
    toPointSeries,
    addColumn,
...
const assertSeriesFloatEquality = (series1: Series, series2: Series) => {
    series1.forEach((point, i) => {
        expect(point).toBeCloseTo(series2[i])
    })
}
...
describe('createSeries', () => {
    it('should create series []', () => {
        ...
        const series = createSeries(0, () => 1)

        expect(series).toEqual(expectedOutcome)
    })

    it('should create series [1, 1, 1]', () => {
        const expectedOutcome = [1, 1, 1]
        const series = createSeries(3, () => 1)

        expect(series).toEqual(expectedOutcome)
    })

    it('should create series [1, 2, 3, 4, 5]', () => {
        const expectedOutcome = [1, 2, 3, 4, 5]
        const series = createSeries(5, (i) => i + 1)

        expect(series).toEqual(expectedOutcome)
    })

    it('should create series [1, 2.1, 3.241]', () => {
        const expectedOutcome = [1, 2.1, 3.241]
        const series = createSeries(3, (i) => 2.1 ** Math.log2(i + 1))

        assertSeriesFloatEquality(series, expectedOutcome)
    })

    it('should throw error if length is less than zero', () => {
        expect(() => createSeries(-1, () => 1)).toThrowError(/^Series length must be greater or equal zero!$/)
    })
})
...
describe('transformSeries', () => {
    it('should transform [1, 2, 3] to [1, 1, 1]', () => {
        ...
        const transformedSeries = transformSeries([1, 2, 3], () => 1)

        expect(transformedSeries).toEqual(expectedOutcome)
    })

    it('should transform [1, 2, 3] to [100, 200, 300]', () => {
        const expectedOutcome = [100, 200, 300]

        const transformedSeries = transformSeries([1, 2, 3], (n) => n * 100)

        expect(transformedSeries).toEqual(expectedOutcome)
    })

    it('should transform [1, 2, 3, 4, 5] to [1, 4, 9, 16, 25]', () => {
        const expectedOutcome = [1, 4, 9, 16, 25]

        const transformedSeries = transformSeries([1, 2, 3, 4, 5], (n, i) => n * (i + 1))

        expect(transformedSeries).toEqual(expectedOutcome)
    })

    it('should keep input series [1, 2, 3] unchanged', () => {
        const inputSeries = [1, 2, 3]
        const expectedOutcome = [1, 2, 3]

        transformSeries(inputSeries, (n) => n * 100)

        expect(inputSeries).toEqual(expectedOutcome)
    })
})
