import type { PartialArgs, PlayState } from './types'

type PartialConstructorArgs = PartialArgs & {
...
export class Partial {
    ...
    public constructor(args: PartialConstructorArgs) {
    ...
    public play(pitch: number, time = this._audioContext.currentTime) {
        if (this._state === 'used') throw new Error(`Attempting to start partial with 'used' state`)

        const frequency = this._rate * pitch
        if (frequency > 20000 || frequency < 20) throw new Error(`Partial frequency="${frequency}" is out of range`)

        this._pitch = pitch
        this.setOscFrequency(frequency, time)
    ...
    public setRate(rate: number) {
        ...
        if (this._state === 'playing' && this._pitch) {
            this.setOscFrequency(this._rate * this._pitch)
        }
    }
    ...
    private setOscFrequency(frequency: number, time = this._audioContext.currentTime) {
